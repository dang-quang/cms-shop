...
function AddPromotionPage() {
  ...
  const FORM_CELL_CUSTOM = [
    ...
    {
      id: 'vi',
      value: ['Tên chương trình khuyến mãi', 'Thời gian', 'Gian hàng áp dụng khuyến mãi'],
    },
  ];
  ...
  const getShop = async () => {
    ...
    setSelectedChannel(res.data[0].name);
    setShopData(res.data[0].shop_data);
    setValues({ ...values, ['shop_id']: res.data[0].shop_data[0].shopId });
  };
  ...
  useEffect(async () => {
    ...
    res.data.item_list.map((item, index) => {
      ...
      if (selectedChannel === 'Lazada') {
        ...
        if (!item.detail.model_data) {
          ...
          item.detail.model_data.model.map((subItem, subIndex) => {
            ...
            sku.price = subItem.price_info[0].original_price;
            sku.stock = subItem.stock_info[0].current_stock + subItem.stock_info[1].current_stock;
            sku.decrease_price = subItem.price_info[0].original_price;
            sku.promotion_qty = sku.stock;
            sku.status = true;
            list_sku.push(sku);
      ...
      product.image =
        selectedChannel === 'Lazada' ? item.detail.images[0] : item.detail.image.image_url_list[0];
  ...
  const handleUpdateAddedProduct = () => {
    ...
    checked_2.map((item, stt) => {
      ...
      data[stt].list_sku.map((props, number) => {
        ...
        if (batchSetting.discount >= 0 && batchSetting.discount.length > 0) {
          cloneItem['decrease_price'] =
            cloneItem.price - (Number(batchSetting.discount) * cloneItem.price) / 100;
        }
        // Update Quantity Promotion
        if (batchSetting.quantity_type == 'limited' && batchSetting.quantity_value > 0) {
      ...
      if (batchSetting.limit_type == 'limited' && batchSetting.limit_value > 0) {
  ...
  const handleSubmit = async () => {
    let isValidTime = moment(values.time_to).unix() > moment(values.time_from).unix();
    if (!values.shop_id) {
      // validate shop
    ...
    return (
      <React.Fragment>
        <TableRow key={index} className={tableClasses.tableBodyRow}>
          ...
          <TableCell className={tableClasses.tableCell}>
            <p className={classes.text + ' ' + classes.infoTextPrimary}>{stock}</p>
          </TableCell>
        </TableRow>
      </React.Fragment>
    );
  ...
  const renderTable = (item, index) => {
    return (
      <>
        <TableRow className={tableClasses.tableBodyRow} style={{ backgroundColor: '#f9f9f9' }}>
          <TableCell className={tableClasses.tableCell}>
            <Checkbox
            ...
            <div className={classes.cellInfo}>
              ...
              <div className={classes.infoTextContainer}>
                <Tooltip title={item.name} placement="right-start">
                  <p
                    className={[classes.text, classes.infoTextPrimary, classes.textLineClamp].join(
                      ' '
                    )}>
                    {item.name}
                  </p>
          ...
          <TableCell className={tableClasses.tableCell}></TableCell>
          <TableCell className={tableClasses.tableCell}></TableCell>
          <TableCell className={tableClasses.tableCell} style={{ width: '200px' }}>
        ...
        <TableRow className={tableClasses.tableBodyRow}>
          ...
          <TableCell className={tableClasses.tableCell}>
            {values.list_products[index].list_sku.map((props, stt) => {
              return (
                <p
                  className={
                    classes.text + ' ' + classes.infoTextPrimary + ' ' + classes.classifiedMargin
                  }>
          ...
          <TableCell className={tableClasses.tableCell}>
            {values.list_products[index].list_sku.map((props, stt) => {
              return (
                <FormControl component="fieldset" size="small" style={{ width: '100%' }}>
                  <OutlinedInput
                    id="outlined-adornment-amount-5"
                    value={props.decrease_price}
                    onChange={handleChangeClassifiedValue(props, index, 'decrease_price')}
                    autoComplete="off"
                    type="number"
                    placeholder={text.placeholder}
                    className={classes.FieldMaxWidth + ' ' + classes.classifiedMargin2}
                  />
                </FormControl>
              );
            })}
              ...
              return (
                <FormControl component="fieldset" size="small" style={{ width: '100%' }}>
                  <OutlinedInput
                    ...
                    onChange={handleChangeClassifiedValue(props, index, 'promotion_qty')}
                    autoComplete="off"
                    type="number"
                    placeholder={text.placeholder}
                    className={classes.FieldMaxWidth + ' ' + classes.classifiedMargin2}
              ...
              return (
                <div className={classes.classifiedMargin}>
                  <Switch
                    ...
                    onChange={handleChangeClassifiedValue(props, index, 'status', 'bool')}
                    name="checkedA"
                  />
                </div>
              );
    ...
    return (
      <div className={classes.batchSetting}>
        <div className={classes.batchSettingText + ' ' + classes.flex_center}>
          <p className={classes.batchSettingText_Title}>{text.batch_setting.text[0]}</p>
        ...
        <div className={classes.flex_center} style={{ alignItems: 'end' }}>
              ...
              <div className={classes.flex_center}>
                <FormControl variant="outlined" size="small">
                  <Select
                    ...
                    <MenuItem value={'unchanged'}>{text.batch_setting.select[0]}</MenuItem>
                    <MenuItem value={'unlimited'}>{text.batch_setting.select[1]}</MenuItem>
                    <MenuItem value={'limited'}>{text.batch_setting.select[2]}</MenuItem>
                  </Select>
                </FormControl>
          ...
          <div className={classes.batchSettingAction + ' ' + classes.flex_center}>
    ...
    <Card>
      ...
      <CardBody className={classes.cardBody}>
        ...
        <FormGroupCustom title={text.form_group_custom[0]}>
          ...
          {/* Combo Name */}
          <FormCellCustom label={text.form_cell_custom[0]} helperText={text.helper_text[0]}>
        ...
        <FormGroupCustom
          ...
          subTitle={values.list_products?.length + ' ' + text.form_group_custom_subtitle[0]}
          hasButton={true}
        ...
        <ModalCustom
          ...
          <div className={tableClasses.tableScrollModal} style={{ marginTop: '0' }}>
          ...
          <div
            className={[tableClasses.flex_center_between, tableClasses.paginationContainer].join(
              ' '
            )}>
...
AddPromotionPage.layout = Admin;

export default WithAuthentication(AddPromotionPage);
