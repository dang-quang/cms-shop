...
function PurchaseOrderDetailPage() {
  ...
  const getPOInfo = async () => {
    let params = {};
    params._id = id;
    var res = await getAllPurchaseOrder(params);
    let stock = await getAllStock();
    ...
    stock.data.map((item, index) => {
      if (item._id == res.data.data[0].stock_id) {
        setStockInfo(item);
      }
    });
...
PurchaseOrderDetailPage.layout = Admin;

export default WithAuthentication(PurchaseOrderDetailPage);
