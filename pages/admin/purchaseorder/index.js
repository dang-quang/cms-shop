  ...
  const renderTable = (item, index) => {
    ...
    return (
      <TableRow key={index} className={tableClasses.tableBodyRow} key={index}>
        ...
        <TableCell className={tableClasses.tableCell}>
          <p className={classes.text + " " + classes.infoTextPrimary} style={{color: item.status === "draft" ? grayColor[0] : primaryColor[0] }}>
            {item.status}
          </p>
        </TableCell>
        <TableCell className={tableClasses.tableCell}>
  ...
  return (
    <Card>
      ...
      <CardBody className={classes.cardBody}>
        ...
        >
          <div
            className={dashClasses.filterSelections + " " + classes.flex_center}
          >
